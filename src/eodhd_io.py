""" eodhd_io.py ----------- Round-trip helpers for EODHD daily and intraday OHLCV data between:
...
EXCHANGE_INFO: dict[str, dict] = {
    "LSE": {"calendar": "XLON", "tz": "Europe/London", "open": time(8, 0), "close": time(16, 30)},
    "US": {"calendar": "XNYS", "tz": "America/New_York", "open": time(9, 30), "close": time(16, 0)},
    "AU": {"calendar": "XASX", "tz": "Australia/Sydney", "open": time(10, 0), "close": time(16, 0)},
}
...
def _n_sessions_before(cal: ec.ExchangeCalendar, ref_date: date, n: int) -> date:
...
_INSERT_DAILY = """
INSERT OR REPLACE INTO {tablename} (code, timestamp, datetime, date, op, hi, lo, cl, ac, vo)
VALUES (:code, :timestamp, :datetime, :date, :op, :hi, :lo, :cl, :ac, :vo);
"""
...
def _parse_ohlcv(df: pd.DataFrame, has_ac: bool) -> pd.DataFrame:
    ...
    rename = {"Open": "op", "High": "hi", "Low": "lo", "Close": "cl", "Volume": "vo"}
    if has_ac:
...
def _start_from_actual_dates(all_dates: list, end_date: date, n: int) -> date:
    """Given a sorted list of trading dates actually returned by EODHD, return the date
    that is n-1 positions before end_date (i.e. so that there are exactly n dates
    from start through end inclusive).
    Unlike _n_sessions_before(), this counts dates that EODHD actually provided —
    including half-days that exchange_calendars omits.
...
def csv2pandas_daily(code: str, csv_path: pathlib.Path) -> pd.DataFrame:
    ...
    merged = skeleton.merge(
        raw[["date", "op", "hi", "lo", "cl", "ac", "vo"]], on="date", how="left"
    )
...
def csv2pandas_intraday(code: str, csv_path: pathlib.Path, interval: str) -> pd.DataFrame:
...
def polars2pandas(df: pl.DataFrame) -> pd.DataFrame:
    ...
    if "ac" in pdf.columns:
        base_cols = ["code", "timestamp", "datetime", "date", "op", "hi", "lo", "cl", "ac", "vo"]
    else:
        base_cols = ["code", "timestamp", "datetime", "local_date", "op", "hi", "lo", "cl", "vo"]
...
class Database:
    ...
    def _date_range_in_table(
        self, tablename: str, code: str, is_daily: bool
    ...
    def from_csv(
        self, code: str, csv_path: pathlib.Path, interval: str, tablename: str
    ) -> None:
    ...
    def fetch(
        ...
        from_date / to_date are Python date objects.
        If omitted, EODHD returns its full default window (last 120 days for intraday).
        """
        token = self._require_token()
        if _is_intraday(interval):
            from_ts = (
                int(datetime(from_date.year, from_date.month, from_date.day).timestamp())
                if from_date
                else None
            )
            to_ts = (
    ...
    def to_pandas(
        ...
        If code and interval are supplied, the method checks whether the requested
        date range is present in the database and fetches from EODHD if not.
        ...
        end : last date to return (Python date); defaults to cached_max (not date.today())
